...
urlpatterns = [
    ...
    path("items/delete/<int:item_id>", ItemDeleteView.as_view(), name="delete-item"),
    path("items/<int:item_id>", ItemDetailView.as_view(), name="item-detail"),
    path("items", ItemListAllView.as_view(), name="item-list-all"),
    path(
        "items/profile/<str:filter_group>",
        ProfileItemsAPIView.as_view(),
        name="profile-item-list",
    ),
    path(
        "items/<int:item_id>/reactivate",
        ReactivateItemAPIView.as_view(),
        name="reactivate_item",
    ),
    path(
        "items/<int:item_id>/sold",
        MarkItemAsSoldAPIView.as_view(),
        name="mark_item_as_sold",
    ),
    path(
        "purchase-requests/create",
        CreatePurchaseRequestAPIView.as_view(),
        name="create-purchase-request",
    ),
    path(
        "purchase-requests/accept/<int:purchase_request_id>",
        AcceptPurchaseRequestAPIView.as_view(),
        name="accept-purchase-request",
    ),
    path(
        "purchase-requests/item/<int:item_id>",
        GetPurchaseRequestsForItemView.as_view(),
        name="get-purchase-requests-for-item",
    ),
    path(
        "purchase-requests/buyer/<int:item_id>",
